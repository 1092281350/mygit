...
int CompareFeasibilityTests(Trajectory* mytrajectory){
   int kNumSegments = 30;
  const  int kN = 12;
  const int kD = 4;
  Segment::Vector segments(kNumSegments, Segment(kN, kD));
  if(mytrajectory==nullptr){
    ROS_INFO("test feas node  trajectory is null-------");
    std::srand(1234567);
...
pos_trajectory.getTrajectoryWithAppendedDimension(
        yaw_trajectory, &trajectory);
  ...
  for (size_t i = 0; i < segments.size() ; i++) {
    ...
    ROS_INFO("feasibility is %d",result_sampling_01[i]);
  }
...
void getMyTraj(const mav_planning_msgs::PolynomialTrajectory &msg ){

  ROS_INFO("GET traj---------------------");
  myset=true;
}
int main(int argc, char** argv) {
  ros::init(argc, argv, "test_feas_node");
  ros::NodeHandle nh;
  CompareFeasibilityTests(nullptr);
  ros::Subscriber traj_sub=nh.subscribe("trajectory",1,&getMyTraj);
