...
uint32_t pwm_gain=0;
bool ok=false;
void local_velocity(const geometry_msgs::Vector3Stamped::ConstPtr& msg){
 pwm_gain= (msg->vector.x+msg->vector.y)*7000;
...
ros::Subscriber localv = nh.subscribe("dji_sdk/velocity", 10, &local_velocity);
...
bool pwmDemo() {
  ...
  initOnTimeUs = 6000+pwm_gain; //us, 30 percent duty cycle
