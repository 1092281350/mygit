...
// ros::Subscriber controlStatusSub = nh.subscribe("/dtu_controller/control_status", 1, &myCallback);
  ...
  // ros::Timer loopTimer = nh.createTimer( ros::Duration(0.05),calculateDistanceCallback);
     ...
     int angle=0;
  bool takeoff_result;
  ros::param::get("height",altitude);
  ros::param::get("angle",angle);
  ros::param::get("isTakeOff",isTakeOff);
...
spinner.stop();
  ...
  takeoff_result = isTakeOff ? monitoredTakeoff():true;
  ...
  if(takeoff_result)
  {
    ...
    square_mission.setTarget(3.1147,-2.7280,altitude, angle);
