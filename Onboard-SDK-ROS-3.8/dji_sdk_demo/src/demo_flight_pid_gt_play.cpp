...
void callback(const ros::TimerEvent& event)
{
  std_msgs::Float32MultiArray mypx,mypy,desp,desv,desa;
      sensor_msgs::Joy controlPosYaw;
    //距离目标点小于0.2m,进行下一阶段
    // calculate_dis();
if(usepid){
    ...
    desp.data.push_back(des_pos_[0]);
    desp.data.push_back(des_pos_[1]);
    despos_pub.publish(desp);
  //发布期望速度
...
despos_pub = nh.advertise<std_msgs::Float32MultiArray>("/my/des_pos", 10);
