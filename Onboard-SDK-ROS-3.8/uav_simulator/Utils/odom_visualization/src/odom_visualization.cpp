...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  // velROS.pose.position.z = pose(2);
    velROS.pose.position.x = current_local_pos.x;
  velROS.pose.position.y = current_local_pos.y;
  velROS.pose.position.z = current_local_pos.z;
...
void dji_local_callback(const geometry_msgs::PointStamped::ConstPtr& msg){
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber sub_cmd  = n.subscribe("cmd",  100,  cmd_callback);
      local_pos_sub=n.subscribe("/dji_sdk/local_position", 10, dji_local_callback);
