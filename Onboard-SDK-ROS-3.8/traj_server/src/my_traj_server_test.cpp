...
using namespace trajectory_generation_helper::circles;
...
//  test  circle traj pub
void pubcmdCallback(const ros::TimerEvent &e){
    quadrotor_common::Trajectory traj=computeHorizontalCircleTrajectory(Eigen::Vector3d(0.0,0.0,1.2),10.0,1.0,0.0,1.0,50);
    int i=0;
    std::list<quadrotor_common::TrajectoryPoint>::const_iterator p1=traj.points.begin();
    for(p1; p1 != traj.points.end(); p1++){
        ros::Time time_now = ros::Time::now();
        ...
        cmd.position.x = p1->position[0];
        cmd.position.y = p1->position[1];
        cmd.position.z = p1->position[2];

        cmd.velocity.x =p1->velocity[0];
        cmd.velocity.y = p1->velocity[1];
        cmd.velocity.z = p1->velocity[2];

        cmd.acceleration.x = p1->acceleration[0];
        cmd.acceleration.y = p1->acceleration[1];
        cmd.acceleration.z = p1->acceleration[2];

        cmd.jerk.x =p1->jerk[0];
        cmd.jerk.y = p1->jerk[1];
        cmd.jerk.z = p1->jerk[2];

        cmd.yaw = p1->heading;
        cmd.yaw_dot = p1->heading_rate;
...
{
    ros::init(argc, argv, "my_traj_server_test");
    ros::NodeHandle nh("~");

    pose_cmd_pub = nh.advertise<quadrotor_msgs::PositionCommand>("/planning/pos_cmd", 50);
    traj_pts_pub = nh.advertise<geometry_msgs::PoseArray>("/planning/traj_pts", 50);
    poly_traj_sub = nh.subscribe("/planning/poly_coefs", 10, polyTrajCallback);


    // ros::Timer cmd_timer = nh.createTimer(ros::Duration(0.01), cmdCallback);
    ros::Timer cmd_timer = nh.createTimer(ros::Duration(0.01), pubcmdCallback);
    /* control parameter */
    cmd.kx[0] = pos_gain[0];
    cmd.kx[1] = pos_gain[1];
    cmd.kx[2] = pos_gain[2];
